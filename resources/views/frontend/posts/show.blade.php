...
@section('content')
    <article class="bg-white rounded-lg shadow p-8">
            ...
            <div class="mt-8 pt-8 border-t">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($post->tags as $tag)
                        <a href="#" class="px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-sm hover:bg-blue-200">
                            #{{ $tag->name }}
                        </a>
                    @endforeach
                </div>
        ...
        <form method="POST" action="{{ route('comment.store', $post) }}" id="commentForm">
            ...
            <textarea name="content" required class="border rounded p-2 w-full mb-2" rows="4"
