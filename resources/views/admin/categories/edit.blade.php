    ...
    <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                @if($category->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $category->image) }}" alt="Category Image" class="h-20 w-auto rounded">
                    </div>
                @endif
            ...
            <div class="mb-4">
                ...
                <textarea name="meta_description" id="meta_description" rows="3"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('meta_description', $category->meta_description) }}</textarea>
            </div>
            ...
            <div class="mb-4 flex gap-6">
                <label class="flex items-center">
                    <input type="checkbox" name="index" value="1" {{ old('index', $category->index) ? 'checked' : '' }}
                        class="mr-2">
                    <span class="text-gray-700 text-sm font-bold">Index (Cho phép Google lập chỉ mục)</span>
                </label>
                <label class="flex items-center">
                    <input type="checkbox" name="follow" value="1" {{ old('follow', $category->follow) ? 'checked' : '' }}
                        class="mr-2">
