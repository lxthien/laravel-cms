...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center">
                <div class="h-12 w-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white text-lg font-bold">
            ...
            @endphp
            <span class="px-4 py-2 text-lg font-semibold rounded-full {{ $badgeColors[$activityLog->log_name] ?? 'bg-gray-100 text-gray-800' }}">
                {{ $labels[$activityLog->log_name] ?? $activityLog->log_name }}
            </span>
        </div>
    ...
    @if($activityLog->properties)
        <div class="bg-white rounded-lg shadow p-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="mt-1 p-2 bg-red-50 rounded border border-red-200">
                                        <code class="text-sm text-red-900">{{ is_array($change['old']) ? json_encode($change['old']) : $change['old'] }}</code>
                                    </div>
                                </div>
                                <div>
                                    <label class="text-sm text-gray-600">Giá trị mới:</label>
                                    <div class="mt-1 p-2 bg-green-50 rounded border border-green-200">
                                        <code class="text-sm text-green-900">{{ is_array($change['new']) ? json_encode($change['new']) : $change['new'] }}</code>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            @else
                <div class="bg-gray-50 rounded-lg p-4">
                    <pre class="text-sm font-mono text-gray-900 overflow-x-auto">{{ json_encode($activityLog->properties, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </div>
            @endif
        </div>
    @endif
    ...
    @if($activityLog->user_agent)
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <pre class="text-sm font-mono text-gray-900 overflow-x-auto break-words">{{ $activityLog->user_agent }}</pre>
            </div>
        </div>
    @endif
                ...
                <div class="space-y-3">
                    @foreach($relatedLogs as $log)
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            @endphp
                            <span class="ml-4 px-2 py-1 text-xs font-semibold rounded {{ $badgeColors[$log->log_name] ?? 'bg-gray-100' }}">
